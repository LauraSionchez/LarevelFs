    ...
    function Plugin(element, options) {
        ...
        function init() {
            ...
            var getListWidth = $.map($el.children('li:not(.nav-title)'), function (val) {
                ...
                _getValues = function () {
                    wrapperWidth = $('#' + options.wrapperId).outerWidth();
                    /* incase its changed we get it again */
...
function pageSetUp() {
    ...
    helpers.get_icon = function (data, d, row) {
        if (data != '') {
    ...
    helpers.get_color = function (data, d, row) {
        if (data != '') {
            return '<i class="fa fa-square" aria-hidden="true" style="color: ' + data + '!important;"></i>';
        } else {
            return '';
        }
    };

    helpers.get_avatar = function (data, d, row) {
        if (data !== 'default') {
            return '<img src="/img/avatar/'+data+'" alt="IMG" class="img-thumbnail" style="max-width:50px;">';
        } else {
            return '';
        }
    };
